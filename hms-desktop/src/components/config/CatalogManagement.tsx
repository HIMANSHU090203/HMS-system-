...
const CatalogManagement = ({ user }: { user: any }) => {
  const [activeTab, setActiveTab] = useState<'allergies' | 'conditions' | 'diagnoses' | 'medicines'>('allergies');
  ...
  const [formData, setFormData] = useState<Partial<Allergy & ChronicCondition & Diagnosis & Medicine>>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      switch (activeTab) {
        case 'allergies':
          await catalogService.addAllergy(formData as Omit<Allergy, 'id' | 'isActive'>);
          break;
        case 'conditions':
          await catalogService.addChronicCondition(formData as Omit<ChronicCondition, 'id' | 'isActive'>);
          break;
        case 'diagnoses':
          await catalogService.addDiagnosis(formData as Omit<Diagnosis, 'id' | 'isActive'>);
          break;
        case 'medicines':
          await catalogService.addMedicine(formData as Omit<Medicine, 'id' | 'isActive'>);
          break;
      }
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      (['allergies', 'conditions', 'diagnoses', 'medicines'] as const).map(tab => React.createElement(
        'button',
        {
          key: tab,
