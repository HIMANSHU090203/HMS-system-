...
const SurgeryScheduling = ({ user, onBack }) => {
  const [surgeries, setSurgeries] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    otService.getSurgeries({ limit: 30 }).then((res) => {
  ...
  return (
    <div style={{ padding: 16, backgroundColor: '#F0F0F0' }}>
      <h2 style={{ marginBottom: 16 }}>Surgery Scheduling</h2>
      {error && <p style={{ color: '#B91C1C', marginBottom: 12 }}>{error}</p>}
      <p style={{ color: '#6B7280', marginBottom: 16 }}>View and manage scheduled surgeries. Use Surgery Management for full details and updates.</p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
              ...
              <th style={{ textAlign: 'left', padding: 12 }}>OT Room</th>
            </tr>
          </thead>
          <tbody>
            {surgeries.map((s) => (
              <tr key={s.id} style={{ borderBottom: '1px solid #E5E7EB' }}>
                ...
                <td style={{ padding: 12 }}>{s.operationTheatre?.name ?? '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      {surgeries.length === 0 && !error && <p style={{ color: '#6B7280', padding: 16 }}>No surgeries scheduled.</p>}
    </div>
  );
};
