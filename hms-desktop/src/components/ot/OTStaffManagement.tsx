...
const OTStaffManagement = ({ user, onBack }) => {
  ...
  return (
    <div style={{ padding: 16, backgroundColor: '#F0F0F0' }}>
      <h2 style={{ marginBottom: 16 }}>OT Staff Management</h2>
      {error && <p style={{ color: '#B91C1C', marginBottom: 12 }}>{error}</p>}
      <p style={{ color: '#6B7280', marginBottom: 16 }}>Assign surgeons, anesthesiologists, nurses, and technicians to scheduled surgeries.</p>
      ...
      {surgeries.length === 0 && !error && <p style={{ color: '#6B7280', padding: 16 }}>No scheduled surgeries.</p>}
    </div>
  );
};

export default OTStaffManagement;
